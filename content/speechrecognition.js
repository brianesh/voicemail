if (!("webkitSpeechRecognition" in window) && !("SpeechRecognition" in window)) {
    ...
    class VoiceEmailAssistant {
        ...
        async markEmailsByDay(folder, day, read = true) {
            try {
                const accessToken = await this.ensureValidToken();
                ...
                const dateStr = this.formatDateForQuery(targetDate);
                const nextDateStr = this.formatDateForQuery(nextDay);
                ...
                this.speak(`Marked emails from ${day} as ${read ? 'read' : 'unread'}`);
                this.addToHistory(`Mark emails from ${day}`, `Status updated to ${read ? 'read' : 'unread'}`);
            } catch (error) {
                ...
                this.speak(errorMessage);
                this.addToHistory("Mark emails by day", errorMessage);
                throw error;
            }
        }
        ...
        async handleMarkStatusCommand(filter, status, folder) {
            if (filter === 'all') {
                ...
                await this.markEmailsByDay(folder, day, status === 'read');
                this.speak(`Marked emails ${filter} in ${folder} as ${status}`);
                this.addToHistory(`Mark emails ${filter} in ${folder} as ${status}`, "Status updated");
            } else {
